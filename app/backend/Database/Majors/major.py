...
class Major(Base):
    ...
    average_wage = Column(Integer)
    total_degrees_awarded_in_2015 = Column(String)
    total_people_in_work_foce = Column(Integer)
    average_age_work_force = Column(String)
    cities_high_graduates_2015 = relationship(
        "City", secondary=majors_top_city_association)
    universities_high_graduates_2015 = relationship(
        "University", secondary=major_top_university_association)
