...
def city_basic_populate():
    ...
    with open('../scrapers/cities.json', 'r') as f:
    ...
    for city in city_data:
        ...
        city_db.add_basic_city_data(
            city["image_description"], city["city_image"], city["county_id"], city["county_name"])
...
def add_major_relationship():
    ...
    session = Session()
    for city in city_data:
        city_db = session.query(City).filter(
            City.id == city["city_id"]).first()
        top_majors_objects = []

        for major_id in city["top_grad_majors"]:
            if major_id != "data_year":
                major = session.query(Major).filter(
                    Major.id == major_id).first()
...
def print_cities():
    ...
    for city in cities:
        universities = session.query(University).filter(
            University.city == city).all()
        print(str(city.city_name) + " has id " + city.id +
              " and universities " + universities[0].name)
    session.commit()
    session.close()
