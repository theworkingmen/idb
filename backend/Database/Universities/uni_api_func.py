...
def get_uni(sort_tut, sort_name, f_type, state):
    ...
    universities = parseInputs(
        sort_tut, sort_name, f_type, state, universities)
    ...
    if sort_name == 'Desc':
        ...
        if sort_tut == 'Asc':
            ...
            universities = universities.order_by(
                University.state_tuition).all()
        else:
            # Sort by in-state tuition, descending
            universities = universities.order_by(
                University.state_tuition.desc()).all()
    else:
